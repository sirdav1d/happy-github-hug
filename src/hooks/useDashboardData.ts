...
export const useDashboardData = (
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [dashboardResult, salesResult, leadsResult] = await Promise.all([
        ...
        supabase
          .from('sales')
          .select('amount, is_new_client, acquisition_cost, sales_count, attendances')
          .eq('user_id', userId),
        supabase
          .from('leads')
      ...
      if (data) {
        ...
        // Sum sales_count for actual number of individual sales
        const totalSalesCount = salesData.reduce(
          (sum, s) => sum + Number(s.sales_count || 1),
          0,
        );
        ...
        const totalAcquisitionCost = salesData.reduce(
          (sum, s) => sum + Number(s.acquisition_cost || 0),
          0,
        );
        ...
        // Sum attendances for conversion calculation
        const totalAttendances = salesData.reduce(
          (sum, s) => sum + Number(s.attendances || 0),
          0,
        );

        // Taxa de conversão baseada em atendimentos se disponível, senão leads
        let conversionRate = 0;
        if (totalAttendances > 0) {
          // Real conversion: sales / attendances
          conversionRate = (totalSalesCount / totalAttendances) * 100;
        } else {
          // Fallback to lead-based conversion
        ...
        const rawKpis =
          data.kpis && typeof data.kpis === 'object'
            ? (data.kpis as Partial<KPI>)
            : {};
        const storedKpis: KPI = {
          ...defaultKPIs,
          ...rawKpis,
        };
        ...
        const dynamicKpis: KPI = {
          ...storedKpis,
          averageTicket:
            averageTicket > 0 ? averageTicket : storedKpis.averageTicket,
          totalSalesCount:
            totalSalesCount > 0 ? totalSalesCount : storedKpis.totalSalesCount,
          totalAttendances:
            totalAttendances > 0 ? totalAttendances : storedKpis.totalAttendances,
          cac: cac > 0 ? cac : storedKpis.cac,
          ltv: ltv > 0 ? ltv : storedKpis.ltv,
          conversionRate:
            conversionRate > 0
              ? conversionRate
              : storedKpis.conversionRate ?? 0,
        };
        ...
        const parsedData: DashboardData = {
          ...
          appSettings:
            (data.app_settings as unknown as DashboardData['appSettings']) || {
              aggressiveMode: false,
              considerVacation: false,
            },
          kpis: dynamicKpis,
          historicalData:
            (data.historical_data as unknown as MonthlyData[]) || [],
          currentYearData:
            (data.current_year_data as unknown as MonthlyData[]) || [],
  ...
  const saveData = async (
    ...
    try {
      ...
      if (data.historicalData !== undefined)
        updateData.historical_data = data.historicalData as unknown as Json;
      if (data.currentYearData !== undefined)
        updateData.current_year_data = data.currentYearData as unknown as Json;
      if (data.team !== undefined)
        updateData.team = data.team as unknown as Json;
      if (data.companyName !== undefined)
        updateData.company_name = data.companyName;
      if (data.businessSegment !== undefined)
        updateData.business_segment = data.businessSegment;
      ...
      if (data.mentorshipStartDate !== undefined)
        updateData.mentorship_start_date = data.mentorshipStartDate || null;
      if (data.appSettings !== undefined)
        updateData.app_settings = data.appSettings as unknown as Json;
  ...
  const mergeData = async (
    ...
    const mergedHistorical = [...dashboardData.historicalData];
    const mergedCurrent = [...dashboardData.currentYearData];
    const mergedTeam = [...dashboardData.team];
    ...
    const mergedYears = Array.from(
      new Set([
        ...(dashboardData.yearsAvailable || []),
        ...(newData.yearsAvailable || []),
      ]),
    ).sort();
